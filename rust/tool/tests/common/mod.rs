...
use lanzaboote_tool::esp::SystemdArchitectureExt;
...
pub fn setup_toplevel(tmpdir: &Path) -> Result<PathBuf> {
    ...
    let test_systemd_stub = format!("{test_systemd}/lib/systemd/boot/efi/{systemd_stub_filename}", systemd_stub_filename=systemd_stub_filename.display());
